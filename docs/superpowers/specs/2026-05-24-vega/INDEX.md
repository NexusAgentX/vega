...
Vega 是一个**不做 RAG、不做 embedding、只存原文 + 独立 metadata 的三层渐进式知识库**，分装为单一二进制，内置 AI Agent，CLI 对外供 coding agent 调用。
...
| 3 | Metadata 筛选 — 不用 RAG，用结构化标签检索 | [03-data-model](./03-data-model.md) |
